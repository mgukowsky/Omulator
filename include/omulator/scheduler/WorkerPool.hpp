...
private:
   ...
   *
   * N.B. we don't lock the workers while doing this, so it's possible that the size of the job_queue
   * can change while this comparison is taking place, but that's an acceptable risk for us,
   * for now at least.
   */
